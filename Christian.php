...
if(isset($_POST['loginAdmin'])) {
    ...
    if($_POST['loginAdmin'] == 1) {
        
        if(strlen($firstname) < 2 || strlen($lastname) < 2) {
           $message.= 'Your name cannot be 1 Letter'; 
           header("Location: http://localhost/quizapp/admin.php?message='$message'");    
           exit();                       
        }
        ...
        else
        {
            ...
            $query = 
            " INSERT INTO `users`" .
            " VALUES('', '$firstname', '$lastname', '$email', '$password', '$username', '$profilepic', '', '$date', '1')";


            if($insertUser = mysqli_query($con, $query)) {
                $_SESSION['email'] = $email;
            
                header("Location: http://localhost/quizapp/admin_page.php");    
                exit();            
            }
            else
            {
